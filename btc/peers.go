...
func (pm *PeerManager) onVerack(p *peer.Peer, msg *wire.MsgVerAck) {
	...
	if !(p.NA().HasService(wire.SFNodeBloom) && p.NA().HasService(wire.SFNodeNetwork) && p.NA().HasService(wire.SFNodeWitness)) ||
		...
		Logger.Warn("Peer %s does not support bloom filtering, diconnecting", zap.String("PEER", p.String()))
		p.Disconnect()
		return
	}
...
func (pm *PeerManager) onReject(p *peer.Peer, msg *wire.MsgReject) {
	Logger.Warn(fmt.Sprintf("Received reject message from peer %d: Code: %s, Hash %s, Reason: %s", int(p.ID()), msg.Code.String(), msg.Hash.String(), msg.Reason))
}
