...
import (
	"github.com/CIP-NL/ontology/common/log"
...
func (ws *WireService) Start() {
	...
	if err != nil {
		Logger.Error("error while starting wire service", zap.Error(err))
	}
	Logger.Info("starting wire service", zap.Uint32("height", best.height))
out:
	for {
		select {
		case m := <-ws.msgChan:
			...
			default:
				Logger.Warn("Unknown message type sent to WireService message chan", zap.Any("MSG", msg))
			}
		case <-ws.quit:
			break out
		}
...
func (ws *WireService) startSync(syncPeer *peerpkg.Peer) {
	...
	if bestPeer != nil {
		...
		Logger.Warn("No sync candidates available")
	}
}
...
func (ws *WireService) handleHeadersMsg(hmsg *headersMsg) {
	peer := hmsg.peer
	if peer != ws.syncPeer {
		Logger.Warn("Received header message from a peer that isn't our sync peer")
		peer.Disconnect()
		return
	}
	_, exists := ws.peerStates[peer]
	if !exists {
		Logger.Warn("Received headers message from unknown peer %s", zap.String("PEER", peer.String()))
		peer.Disconnect()
		return
	}
	...
	if badHeaders > 1 {
		Logger.Warn("Disconnecting from peer %s because he sent us too many bad headers", zap.String("PEER", peer.String()))
		peer.Disconnect()
		return
	}
	...
	if err != nil {
		Logger.Warn("Failed to send getheaders message to peer", zap.String("PEER", peer.String()), zap.Error(err))
		return
	}
}
...
func (ws *WireService) handleMerkleBlockMsg(bmsg *merkleBlockMsg) {
	...
	if peer != ws.syncPeer && !ws.Current() {
		Logger.Warn("Received block from %s when we aren't current", zap.String("PEER", peer.String()))
		return
	}
	state, exists := ws.peerStates[peer]
	if !exists {
		Logger.Warn("Received merkle block message from unknown peer %s", zap.String("PEER", peer.String()))
		peer.Disconnect()
		return
	}
	...
	if _, exists = state.requestedBlocks[blockHash]; !exists {
		...
		// duplicate blocks.
		if ws.params.Name != chaincfg.RegressionNetParams.Name {
			Logger.Warn("Got unrequested block  from %s -- "+
				"disconnecting", zap.ByteString("BLOCK", blockHash.CloneBytes()), zap.String("PEER", peer.Addr()))
			peer.Disconnect()
			return
		}
	}
	...
	if err != nil {
		Logger.Warn("Peer sent an invalid MerkleBlock", zap.String("PEER", peer.String()))
		peer.Disconnect()
		return
	}
	...
	if err == OrphanHeaderError && ws.Current() {
		...
		if state.blockScore < 0 {
			Logger.Warn("Disconnecting from peer %s because he sent us too many bad blocks", zap.String("PEER", peer.String()))
			peer.Disconnect()
			return
		}
		Logger.Warn("Received unrequested block from peer %s", zap.String("PEER", peer.String()))
		return
	} else if err != nil {
		log.Error(err)
		return
...
func (ws *WireService) handleInvMsg(imsg *invMsg) {
	...
	state, exists := ws.peerStates[peer]
	if !exists {
		Logger.Warn("Received inv message from unknown peer %s", zap.String("PEER", peer.String()))
		return
	}
	...
	for _, iv := range invVects {
		...
		if err != nil {
			Logger.Warn("Unexpected failure when checking for "+
				"existing inventory during inv message "+
				"processing", zap.Error(err))
			continue
		}
...
func (ws *WireService) handleTxMsg(tmsg *txMsg) {
	...
	state, exists := ws.peerStates[peer]
	if !exists {
		Logger.Warn("Received tx message from unknown peer %s", zap.String("PEER", peer.String()))
		return
	}
	ht, ok := state.requestedTxns[tx.TxHash()]
	if !ok {
		Logger.Warn("Peer %s is sending us transactions we didn't request", zap.String("PEER", peer.String()))
		peer.Disconnect()
		return
	}
