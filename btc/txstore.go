...
import (
	...
	"github.com/carapace/go/btc/interface"
	"go.uber.org/zap"
	"sync"
	"time"
)
...
func (ts *TxStore) processReorg(lastGoodHeight uint32) error {
	...
	for i := len(txns) - 1; i >= 0; i-- {
		if txns[i].Height > int32(lastGoodHeight) {
			...
			if err != nil {
				Logger.Error("error while getting new hash from string", zap.Error(err))
				continue
			}
			err = ts.markAsDead(*txid)
			if err != nil {
				Logger.Error("error while marking tx as dead", zap.Error(err))
				continue
			}
		}
	}
